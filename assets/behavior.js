...
(function() {
  ...
  visitCurrentNode = function() {
    if (currentNode$.hasClass('folder')) {
      setCurrentNodeExpanded(!currentNode$.hasClass('expanded'));
  ...
  buildNav = function(metaInfo) {
    ...
    $('li', toc$).each(function(index) {
      return buildTOCNode(this, metaInfo);
    });
    return nav$;
  };

  buildTOCNode = function(node, metaInfo) {
    var clickLabel, discloser, label$, node$, targetPath;
    node$ = $(node);
    label$ = node$.find('> .label');
    discloser = label$.find('> .discloser').get(0);
    if (node$.hasClass('file')) {
      targetPath = node$.data('groc').data.targetPath;
  ...
  $(function() {
    var cookiePath, lastMousedownTimestamp, layoutToggle$, metaInfo, setColumnLayout, tocToggle$;
    metaInfo = {
